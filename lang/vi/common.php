<?php

return [
    'action' => [
        'dowload_template' => 'Tải xuống mẫu',
        'upload_excel' => 'Upload Excel',
    ],
    'common_error' => [
        'api_error' => 'Có lỗi xảy ra vui lòng thử lại.',
        'api_not_found' => 'Nguồn không xác định đã xảy ra, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'authorization_header_not_found' => 'Không tìm thấy tiêu đề ủy quyền, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'copy_error' => '[EN] Error when copying',
        'data_exists' => 'Dữ liệu đã tồn tại, vui lòng thử lại.',
        'data_not_fields' => 'Có một số dữ liệu chưa được điền, vui lòng thử lại.',
        'data_not_found' => 'Dữ liệu trống, vui lòng thử lại sau.',
        'delete_file_error' => '[EN] An error occurred while deleting the file',
        'file_not_found' => '[EN] File not found',
        'inappropriate_language' => 'Nội dung không phù hợp với chính sách của hệ thống.',
        'invalid_date' => '[EN] Invalid date',
        'invalid_id' => '[EN] Invalid ID',
        'invalid_or_expired_token' => 'Tài khoản của bạn đã hết hạn hoặc không hợp lệ. Vui lòng đăng nhập lại.',
        'max_content' => 'Nội dung không được vượt quá :max ký tự.',
        'method_not_allowed' => 'API không đúng định dạng, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'not_found' => '[EN] Not found',
        'permission_error' => 'Bạn không có quyền làm tác vụ này, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'program_error' => 'Đã xảy ra lỗi trên chương trình, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'project_unauthorized' => '[EN] Your project or VM has authentication issues, please check again later',
        'refresh_token_fail' => 'Tài khoản của bạn đã hết hạn hoặc không hợp lệ. Vui lòng đăng nhập lại.',
        'request_error' => 'Không nhận được phản hồi từ máy chủ, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'server_error' => 'Đã xảy ra lỗi trên máy chủ, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'unknown_error' => 'Đã xảy ra lỗi, vui lòng liên hệ quản trị viên để được hỗ trợ.',
        'upload_file_error' => '[EN] An error occurred while uploading the file',
        'validation_failed' => 'Dữ liệu không hợp lệ.',
        'vm_error_general' => '[EN] VM is currently unavailable, please contact admin',
    ],
    'common_success' => [
        'add_success' => 'Thêm dữ liệu thành công',
        'all' => 'Tất cả',
        'cancel' => 'Hủy bỏ',
        'close' => 'Đóng',
        'create' => 'Tạo mới',
        'create_and_create_another' => 'Tạo và tạo thêm',
        'delete' => 'Xóa',
        'delete_file_success' => '[EN] Delete file successfully',
        'edit' => 'Sửa',
        'empty' => 'Trống',
        'get_success' => 'Lấy dữ liệu thành công',
        'name' => 'Tên',
        'processing' => 'Đang xử lý...',
        'save' => 'Lưu thay đổi',
        'update_success' => 'Cập nhật dữ liệu thành công',
        'upload_file_success' => '[EN] Upload file successfully',
        'view' => 'Xem',
    ],
    'mark_as_read_success' => 'Đánh dấu đã đọc thành công.',
    'not_init' => 'Chưa khởi tạo',
    'push_token_saved' => 'Lưu thành công.',
    'resource' => [
        'dashboard' => [
            'chart' => [
                'heading' => 'Thống kê check-in & đăng ký',
                'label_checkin' => 'Số check-in',
                'label_registration' => 'Số đăng ký',
                'placeholder_no_event' => 'Vui lòng chọn sự kiện để thống kê dữ liệu',
                'type_day' => 'Theo Ngày',
                'type_hour' => 'Theo Giờ',
            ],
            'choose_event' => 'Vui lòng chọn sự kiện trước khi xuất Excel.',
            'export_excel' => 'Xuất Excel',
            'select' => [
                'chart_type_day' => 'Theo Ngày',
                'chart_type_hour' => 'Theo Giờ (Cộng dồn)',
                'chart_type_label' => 'Loại thống kê',
                'choose_event_label' => 'Chọn sự kiện',
                'end_date_label' => 'Tới ngày',
                'event_placeholder' => '--- Vui lòng chọn một sự kiện ---',
                'heading' => 'Bộ lọc sự kiện',
                'organizer_label' => 'Chọn tổ chức',
                'organizer_placeholder' => '--- Vui lòng chọn một tổ chức ---',
                'start_date_label' => 'Từ ngày',
                'to_day' => '[EN] To Day',
            ],
            'stats' => [
                'attendance_rate_description' => 'So với tổng số đăng ký',
                'attendance_rate_title' => 'Tỉ lệ tham dự',
                'error_message' => 'Không thể tải thống kê',
                'error_title' => 'Lỗi',
                'no_event_message' => 'Vui lòng chọn sự kiện để xem thống kê.',
                'no_event_title' => 'Chưa chọn sự kiện',
                'total_checkin_description' => 'Số người đã check-in',
                'total_checkin_title' => 'Số Check-in',
                'total_registered_description' => 'Số người đăng ký tham gia sự kiện',
                'total_registered_title' => 'Tổng đăng ký',
            ],
        ],
        'setting' => [
            'name' => 'Cấu hình',
        ],
    ],
    'save' => 'Lưu',
    'validation' => [
        'answer_already_exists' => 'Bạn đã gửi câu trả lời cho câu hỏi này rồi.',
        'answers_required' => 'Vui lòng gửi ít nhất một câu trả lời.',
        'notification_id_exists' => 'ID thông báo không tồn tại.',
        'notification_id_integer' => 'ID thông báo phải là số nguyên.',
        'notification_id_required' => 'ID thông báo là bắt buộc.',
        'notification_ids_array' => 'Danh sách ID thông báo phải là mảng.',
        'notification_ids_min' => 'Danh sách ID thông báo phải có ít nhất 1 phần tử.',
        'notification_ids_required' => 'Danh sách ID thông báo là bắt buộc.',
        'notification_type_in' => 'Loại thông báo không hợp lệ.',
        'notification_type_integer' => 'Loại thông báo phải là số nguyên.',
        'option_not_found' => 'Không tìm thấy lựa chọn hợp lệ.',
        'poll_not_available' => 'Poll này không còn hiệu lực hoặc chưa bắt đầu.',
        'question_not_found' => 'Không tìm thấy câu hỏi trong poll.',
        'status_in' => 'Trạng thái không hợp lệ.',
        'status_integer' => 'Trạng thái phải là số nguyên.',
        'user_not_in_poll' => 'Bạn không nằm trong danh sách tham gia poll này.',
    ],
];
